  ...
  Future<void> _collectAudioFiles(Directory dir, List<File> files) async {
    try {
      ...
      await for (final entity in dir.list(
        ...
        if (_ignoredPaths.any((ig) => p.contains(ig))) continue;
        if (excluded.any((ex) => entity.path.startsWith(ex))) continue;

        if (entity is File) {
          final ext = path.extension(entity.path).toLowerCase();
          if (_supportedExtensions.contains(ext)) {
            final fileName = path.basenameWithoutExtension(entity.path);
            if (!RegExp(r'^\d{8,}$').hasMatch(fileName) && !fileName.startsWith('AUD-') && !fileName.startsWith('PTT-')) {
              files.add(entity);
            }
          }
        }
  ...
  Future<void> _loadCachedLibrary() async {
    try {
      ...
      for (final song in cachedSongs) {
        if (song.path != null && File(song.path!).existsSync()) {
          final p = song.path!.toLowerCase();
          final isJunk = p.contains('whatsapp') ||
              p.contains('voice note') ||
              p.contains('recordings') ||
              RegExp(r'^\d{8,}$').hasMatch(song.title.trim());
          if (!isJunk && (song.duration == null || song.duration! >= 15)) {
            validSongs.add(song);
          } else {
            hadInvalid = true;
          }
      ...
      if (hadInvalid || validSongs.length != cachedSongs.length) {
        ...
        notifyListeners();
        debugPrint('Pruned deleted/invalid local tracks from DB. Remaining: ${_songs.length}');
        return;
      }
      ...
      _buildAlbumsAndArtists();
      notifyListeners();
      debugPrint('Loaded ${_songs.length} valid songs from local cache.');
    } catch (e) {
      debugPrint('Error loading local music cache: $e');
    }
  }
