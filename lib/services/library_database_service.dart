...
class LibraryDatabaseService {
  ...
  Future<void> insertOrUpdateArtist(Artist artist) async {
    final db = await database;
    await db.insert(
      'artists',
      _artistToMap(artist),
  ...
  /// Clear only local data from DB.
  Future<void> clearLocalData() async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.delete('songs', where: 'isLocal = ?', whereArgs: [1]);
      await txn.delete('albums', where: 'isLocal = ?', whereArgs: [1]);
      await txn.delete('artists', where: 'isLocal = ?', whereArgs: [1]);
    });
  }
