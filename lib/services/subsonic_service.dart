...
class SubsonicService {
  ...
  static const String _clientName = 'Groovy';
  ...
  Future<AudioSource?> getYoutubeAudioSource(Song song) async {
    final isYt = song.id.startsWith('ytmusic://') ||
        song.id.startsWith('yt_') ||
        (RegExp(r'^[a-zA-Z0-9_-]{11}$').hasMatch(song.id) &&
            !RegExp(r'^[0-9]+$').hasMatch(song.id));
    if (isYt || _youtube != null) {
      _youtube ??= YoutubeService();
      return _youtube!.buildAudioSource(song.id);
    }
    return null;
  }
  ...
  String getCoverArtUrl(String? coverArt, {int size = 600}) {
    if (coverArt != null &&
        (coverArt.startsWith('http://') || coverArt.startsWith('https://'))) {
      var url = coverArt;
