    ...
    let query = `
      SELECT 
        ...
        COALESCE(
          u.last_active_at, 
          u.last_login_at, 
          (SELECT s.last_active_at FROM user_sessions s WHERE s.user_id = u.id ORDER BY s.id DESC LIMIT 1),
          (SELECT s.created_at FROM user_sessions s WHERE s.user_id = u.id ORDER BY s.id DESC LIMIT 1),
          (SELECT h.played_at FROM playback_history h WHERE h.user_id = u.id ORDER BY h.id DESC LIMIT 1),
          u.created_at
        ) as last_active_at,
        COALESCE(
          u.last_login_at,
        ...
        COALESCE(
          ...
          NULLIF(u.last_device, ''), 
          (SELECT NULLIF(s.device_model, '') FROM user_sessions s WHERE s.user_id = u.id AND s.device_model IS NOT NULL ORDER BY s.id DESC LIMIT 1),
