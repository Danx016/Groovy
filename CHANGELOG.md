...
### Fixed & Enhanced
- **Bump de versión forzado para distribución**: Se actualiza el número de versión en todos los archivos de configuración (pubspec.yaml, installer.iss, Runner.rc) para que Windows y Android detecten y descarguen la actualización correctamente desde GitHub Releases.
...
### Fixed & Enhanced
- **Filtro estricto de coincidencia y tolerancia de duración en letras (`LrcLibService`)**:
  - Se eliminó el falso positivo por palabras genéricas en nombres de artistas (`the`, `los`, `las`, `del`, etc.) mediante filtrado por palabras clave y límites léxicos (`\b`).
  - Se redujo la tolerancia máxima de duración de 25s a 12s para evitar asignar remixes, versiones extendidas o temas homónimos.
  - Validación estricta en el catálogo de Genius (`_fetchGeniusLyrics`) con `_isCandidateValid`, descartando automáticamente búsquedas no exactas en vez de devolver el primer resultado.
  - Extracción prioritaria de Closed Captions oficiales de YouTube: cuando el video tiene subtítulos del autor/disquera (`!isAutoGenerated`), se utilizan de inmediato para sincronización al 100% con intros y silencios del video.
- **Restauración de alineación superior en Android (`LyricsListView`)**:
  - En Android/móvil se restauró el inicio natural de la letra desde arriba (`topPadding: 32.0`), eliminando el espacio vacío inicial y permitiendo que los 3 puntos y las primeras estrofas se muestren arriba.
  - La letra fluye de arriba hacia abajo y realiza scroll suave cuando la línea activa alcanza el tercio superior (~28%), preservando en Windows el diseño centrado con la portada.
...
### Fixed & Improved
- **Arquitectura de letras de alto repertorio con sincronización LRC, YouTube CC y respaldo universal (`LrcLibService`, `test/services/lrclib_service_test.dart`)**:
  - **Extracción de subtítulos oficiales sincronizados de YouTube (Closed Captions / CC)**: Para canciones, videos oficiales o temas exclusivos donde no existe archivo `.lrc` comunitario, el motor extrae directamente las pistas de subtítulos incrustadas en el video de YouTube (`youtube_explode_dart`), priorizando español e inglés, removiendo símbolos musicales (`♪`, `♫`) y formateándolas al vuelo como letras sincronizadas línea por línea con marcas de tiempo milimétricas.
  - **Caché persistente universal en disco (0ms de latencia y funcionamiento Offline)**: Todas las letras obtenidas (vía LRCLIB, YouTube CC o Genius) ahora se guardan automáticamente en disco local (`OfflineService.saveLyrics`), y `right_sidebar.dart` y `now_playing_screen.dart` consultan primero el almacenamiento local antes de emitir peticiones por red. Una vez reproducida una canción, su letra carga al instante sin gastar datos móviles y funciona sin conexión a internet.
  - **Repertorio masivo con LRCLIB y tolerancia elástica de duración**: Se incrementó la tolerancia de discrepancia de duración de 8s a 20s para acomodar intros, outros y silencios frecuentes en videos musicales de YouTube, asegurando que canciones populares no sean rechazadas por pequeñas diferencias de tiempo.
  - **Detección inteligente de artistas primarios y colaboraciones (`primaryArtist`)**: Soporte para separar automáticamente colaboraciones complejas (`Bad Bunny, Feid`, `FloyyMenor & Cris MJ`, `Eslabon Armado, Peso Pluma`) buscando tanto al artista principal como a los colaboradores en la metadata combinada.
  - **Limpieza avanzada de ruido en títulos en español e inglés**: Filtrado proactivo de etiquetas como `(Video Oficial)`, `(En Vivo)`, `[Letra]`, `(Audio Oficial)`, `(Visualizer)`, `[4K]` y canales temáticos (`- Topic`, `VEVO`), maximizando la precisión de búsqueda.
  - **Manejo seguro de códigos de estado en Dio (`validateStatus < 500`)**: Se solucionó un defecto crítico donde peticiones con status 404 en la consulta exacta con duración arrojaban una excepción que cancelaba el flujo de fallback sin duración. Ahora se realiza una cascada fluida entre `/get` con duración, `/get` sin duración y `/search` con evaluación ponderada de candidatos.
  - **Integración de Genius como proveedor de respaldo mundial (25+ Millones de canciones)**: Cuando una canción no cuenta con letra sincronizada en LRCLIB ni CC, el sistema consulta automáticamente la API multi-búsqueda de Genius, extrayendo y limpiando en tiempo real las estrofas oficiales desde el contenedor de letras, removiendo etiquetas HTML y traducciones ajenas para que el usuario nunca vea una pantalla de "No hay letra disponible".
  - **Solución integral de letras en pantalla completa en Windows y Android (`NowPlayingScreen`, `DesktopPlayerBar`, `MiniPlayer` y `RightSidebar`)**:
  - **Resiliencia ante telemetría remota de Groovy Connect**: Se eliminó el reseteo cíclico de letras (`_fetchedLyrics = []`) causado por diferencias de ID entre el dispositivo emisor (ej. `dz_...`) y el receptor (ej. YouTube video ID). La comparación de canciones ahora utiliza `NowPlayingScreen.isSameTrack` evaluando título y artista limpios, impidiendo que los pings de telemetría descarten o cancelen las peticiones de letras en curso.
  - **Indexación y caché de doble clave (`song.id` y `artist|title`)**: Las letras cargadas se indexan tanto por ID como por combinación de artista y título normalizados, asegurando que cualquier dispositivo remoto o vista secundaria encuentre la letra al instante sin esperas ni consultas redundantes.
  - **Estabilidad de modo horizontal (Landscape) en Windows**: Se corrigió la condición de colapso donde la pantalla cambiaba indebidamente a la vista centrada sin panel de letras si la letra estaba cargando. Ahora mantiene el diseño dos columnas con indicador de carga y añade un botón de reintento manual si la letra no se encuentra.
  - **Entrega instantánea de letras al abrir pantalla completa (Frame 0)**: Al abrir `NowPlayingScreen` desde la barra inferior de escritorio (`desktop_player_bar.dart`), la barra lateral (`right_sidebar.dart`) o el mini reproductor móvil (`mini_player.dart`), se transfieren las letras en memoria previniendo pantallas en blanco o parpadeos.
  - **Eliminación de `setState` durante `build` en `RightSidebar`**: La actualización de canción en la barra lateral ahora se despacha de forma segura mediante `addPostFrameCallback`, garantizando renderizado fluido a 120 FPS sin advertencias de framework.



- **Solución integral de reproducción remota entre teléfonos vía Groovy Connect (`GroovyConnectService`, `PlayerProvider` y `GroovyConnectModal`)**:
  - **Confirmación inmediata de recepción HTTP (Eliminación de Deadlock y Socket Abort)**: En el servidor HTTP receptor (`/groovy/transfer`), la respuesta `200 OK` se emite inmediatamente tras validar la petición en lugar de esperar la inicialización de ExoPlayer (1.5–3s), evitando que el teléfono emisor exceda el timeout de LAN y aborte el socket TCP con `Connection reset by peer`.
  - **Deduplicación inteligente de transferencias de reproducción**: Se implementó control de idempotencia (`_isDuplicateTransfer`) con ventana de 2.5s para evitar que la llegada concurrente del comando por red local (LAN) y por la nube (Cloud polling) ejecute dos veces consecutivas `playSong`, lo que incrementaba `_playGeneration` y abortaba la carga de la pista a mitad de camino.
  - **Priorización LAN con Fallback a la Nube**: Al transferir una canción, si el dispositivo receptor está en la misma red Wi-Fi, la transferencia se ejecuta por HTTP local (latencia de ~20ms); solo si la transferencia LAN no responde se recurre al servidor en la nube, eliminando comandos duplicados.
  - **Resolución previa de IDs no estándar (Deezer `dz_...` a YouTube)**: Al enviar una canción desde el teléfono controlador, se pre-resuelve el `videoId` real de YouTube para que ambos teléfonos compartan exactamente la misma pista, carátula e identificación.
  - **Sincronización optimista y eliminación de spinner infinito**: `PlayerProvider` ahora sincroniza el estado remoto no solo por ID estricto sino también por coincidencia de título y confirmación activa de reproducción (`isPlaying` o `position > 0`), asegurando que el indicador de carga se oculte inmediatamente cuando el receptor empieza a sonar.
- **Solución integral de descubrimiento en la nube para dispositivos en redes Wi-Fi distintas o datos móviles (`groovy-backend`, `GroovyApiService` y `GroovyConnectService`)**:
  - **Corrección de deduplicación en el Backend (`telemetry.js`)**: En `GET /api/telemetry/playback`, el servidor agrupaba los dispositivos por `platform_deviceModel` en vez de por `device_key` único. Dos teléfonos de la misma marca/modelo o genéricos eliminaban al otro de la respuesta, provocando que un teléfono solo se viera a sí mismo y quedara la lista vacía. Ahora se deduplica estrictamente por `device_key`.
  - **Identificación del dispositivo emisor (`callerDeviceId`)**: El cliente de Flutter ahora envía `?deviceId=$_localDeviceId` para que la consulta SQL en el servidor excluya al dispositivo que consulta y devuelva limpiamente todos los demás dispositivos del usuario.
  - **Recuperación y persistencia transparente del token de sesión**: Si el token en memoria no está inicializado al momento de enviar pings o solicitar dispositivos, `GroovyApiService` y `GroovyConnectService` recuperan automáticamente el token de `StorageService`, evitando que las peticiones se traten como anónimas (`user_id = 0`) donde solo se permitía ver dispositivos con la misma IP pública de router.
  - **Ampliación de ventana activa y timeout resiliente**: Se amplió el timeout de `fetchUserDevices` de 1.8s a 6.0s para soportar latencias en redes móviles (4G/5G/Wi-Fi remota), y se extendió la ventana de dispositivos activos en el backend de 30s a 60s para evitar desconexiones intermitentes por ahorro de batería en Android.
...
- **Solución integral de reproducción para usuarios nuevos en Windows (`YtDlpService`, `PlayerProvider`, `main.dart` e `installer.iss`)**:
  - **Inicialización proactiva de `yt-dlp.exe`**: En el arranque de la app en Windows (`main.dart`), se ejecuta `ensureBinaryAvailable()` para detectar y descargar de inmediato el binario si no existe, sin esperar a que el usuario intente reproducir una canción.
  - **Sincronización sin fallos en primer uso**: Se introdujo coordinación con `Completer` en `YtDlpService`; si un usuario nuevo pulsa Play mientras el binario se está descargando/instalando por primera vez, el reproductor espera activamente la finalización de la descarga en lugar de abortar y fallar.
  - **Descarga directa resiliente sin Rate Limit de GitHub**: Si la API de GitHub devuelve error (por ejemplo HTTP 403 por límite de tasa de 60 peticiones/hora), `updateYtDlp()` descarga directamente desde las URLs de entrega oficial de GitHub sin abortar el proceso.
  - **Manejo seguro de streaming en escritorio**: En `PlayerProvider` y `YoutubeService`, se eliminó el fallback a `setUrl` sin cabeceras HTTP en escritorio (causante de errores 403 en libmpv) y se implementó reintento automático con cabeceras de autenticación completas.
  - **Verificación de Visual C++ Redistributable en `installer.iss`**: El instalador oficial de Windows ahora detecta si Microsoft Visual C++ 2015–2022 (x64) está instalado en el registro y, si falta, lo descarga e instala silenciosamente para que `mpv-2.dll` y `yt-dlp.exe` funcionen al 100% en equipos limpios.
...
### Changed
- Centralizada la versión de producción en `1.1.4`.
