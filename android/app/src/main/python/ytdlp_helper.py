...
def _search_query(target, limit):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        for e in entries[:limit]:
            ...
            thumb_url = _upgrade_thumbnail(thumbnails[-1].get('url')) if thumbnails and isinstance(thumbnails[-1], dict) else ''
    ...
    try:
        ...
        for sec in contents:
            ...
            for item in item_section.get("contents", []):
                ...
                owner_runs = r.get("ownerText", {}).get("runs", []) or r.get("longBylineText", {}).get("runs", [])
                artist = owner_runs[0].get("text", "") if owner_runs else "Unknown Artist"
                
                dur_str = r.get("lengthText", {}).get("simpleText", "0:00")
...
def search_dual(query, limit=20):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        f_classic = executor.submit(_search_youtube_video_innertube, query, limit)

        music_results = f_music.result()
        classic_results = f_classic.result()

        # Fallback to yt-dlp search if Innertube searches return empty
        if not music_results and not classic_results:
            classic_target = f"ytsearch{limit}:{query}"
            classic_results = _search_query(classic_target, limit)
            music_results = _search_query(f"{classic_target} audio", limit)
...
def search(query, limit=25):
    ...
    if not results:
        results = _search_youtube_video_innertube(query, limit)
    if not results:
        results = _search_query(f"ytsearch{limit}:{query}", limit)
